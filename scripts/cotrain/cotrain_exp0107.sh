...
splade_search() {
    python eval_splade.py \
        ...
        --query_lmdb_dir $BASE_DIR/data/lmdb_data/test_queries \
        --idmapping_path $BASE_DIR/data/test_qid_lookup.json \
        --model_name_or_path runs/marcows/dep_warmup_splade/checkpoint-2000 \
        --tokenizer_name bert-base-multilingual-uncased  \
        --task_list sparse \
        --fp16 \
        --per_device_eval_batch_size 1 \
...
encode_query() {
    CUDA_VISIBLE_DEVICES=1 python eval_dense.py \
        ...
        --query_lmdb_dir $BASE_DIR/data/lmdb_data/test_queries \
        --idmapping_path $BASE_DIR/data/test_qid_lookup.json \
        --model_name_or_path runs/marcows/$TRAIN_NAME/checkpoint-2000 \
        --tokenizer_name bert-base-multilingual-uncased  \
        --task_list sent \
        --fp16 \
        ...
        --dataloader_num_workers 32 \
        --q_max_len 32 \
        --embedding_output_dir $BASE_DIR/embeddings/$TRAIN_NAME/checkpoint-2000
}
